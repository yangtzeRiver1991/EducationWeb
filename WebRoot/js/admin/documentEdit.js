$(document).ready(function(){
		...
		$.ajax({
            ...
            success: function(pageResult){
               if(pageResult.status==0){
            	   ...
            	   var doctags = $("#doctags").val();
                   
            	   tagsDiv.append("<ul class=\"tag-level1\">");
            	   for(var i=0;i<tags.length;i++){
            		   if(tags[i].level==1){
            			   if(doctags.indexOf(tags[i].code+',')!=-1){
            				   tagsDiv.append("<li>")
		     			              .append("<input type=\"checkbox\" value=\"")
		     			                  .append(tags[i].code)
		     			              .append("\" name=\"tag\" checked=\"checked\"/>")
		     			                  .append(tags[i].code)
            			   ...
            			   for(var j=0;j<tags.length;j++){ 
            				   if(tags[j].oneLevelCode==tags[i].code){
            					   if(doctags.indexOf(tags[j].code+',')!=-1){
            						   tagsDiv.append("<li>")
			     					              .append("<input type=\"checkbox\" value=\"")
			     					                  .append(tags[j].code)
	...
	$("#doc-add-button").click(function(){
		...
		$.ajax({
            type: "POST",
            url: path+"admin/editDocumentResult.do",
            data: {
              "id":$("#documentId").val(),
           	  "title":title.val(),
           	  "tags":tags.toString(),
